                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase">Merek & Model</th>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse ($kendaraans as $index => $kendaraan)
                                <tr>
                                    <td class="px-6 py-4 text-sm text-gray-600">{{ $kendaraans->firstItem() + $index }}</td>
                                    <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $kendaraan->nama_kendaraan }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-600">{{ $kendaraan->merek }} {{ $kendaraan->model }}</td>
                                    ...
                                    <td class="px-6 py-4 text-sm">
                                        @if ($kendaraan->jenis === 'motor')
                                            <span class="bg-blue-100 text-blue-700 text-xs font-bold px-2 py-1 rounded-full">Motor</span>
                                        @else
                                            <span class="bg-purple-100 text-purple-700 text-xs font-bold px-2 py-1 rounded-full">Mobil</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 text-right text-sm space-x-3">
                                        <a href="{{ route('user.kendaraan.edit', $kendaraan) }}"
                                           class="text-blue-600 hover:underline">Edit</a>
                                        <form action="{{ route('user.kendaraan.destroy', $kendaraan) }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-8 text-center text-gray-500">
                                        ...
                                        <a href="{{ route('user.kendaraan.create') }}" class="text-blue-600 hover:underline ml-1">
                                            Tambah sekarang
                                        </a>
                                    </td>
                                </tr>
